...
#define AM_CMD_ID_NEW_CFG                      ((uint16_t)0x0001) /* NEW_CFG command ID. For more information see AMTaskOnNewConfiguration() function.        */
#define AM_CMD_ID_START_EP                     ((uint16_t)0x0002) /* START_EP command ID. Start the execution phase specified in the parameter of the command.*/
#define AM_CMD_ID_END_OF_EP                    ((uint16_t)0x0003) /* END_OF_EP command ID. Notify the task that an execution phase is ended.                  */
#define AM_CMD_ID_ABORT_AM                     ((uint16_t)0x0004) /* ABORT_AM command ID. Notify the task that the auto mode has been aborted.                */
#define AM_CMD_ID_RESTART_PLAN                 ((uint16_t)0x0005) /* RESTART_PLAN command ID. Notify the task restart the execution plan.                     */
...
sys_error_code_t AMTaskInit(AMTask *_this)
{
  ...
  xRes = xTaskCreate(AMTaskRun, "AMT", AM_TASK_CFG_STACK_DEPTH, s_pxTaskObj, AM_TASK_CFG_PRIORITY, &s_pxTaskObj->m_xTask);
  if (pdPASS == xRes)
  {
          ...
          if (pxTask->m_eState == E_AM_STATE_NOT_STARTED)
          {
            ...
            if (pxAutoModeCfg->bIsValid && pxAutoModeCfg->bEnable)
            {
              ...
              if (pxAutoModeCfg->nStartDelayMS)
              {
                ...
                xTimerChangePeriod(pxTask->m_xIdleTimer, pdMS_TO_TICKS(pxAutoModeCfg->nStartDelayMS), pdMS_TO_TICKS(200));
              }
              else
              {
