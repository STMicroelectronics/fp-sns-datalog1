...
/* Console Service */
static const uint8_t console_service_uuid[16] = {0x1b, 0xc5, 0xd5, 0xa5, 0x02, 0x00, 0xb4, 0x9a, 0xe1, 0x11, 0x0E, 0x00, 0x00, 0x00, 0x00, 0x00};
static const uint8_t console_term_char_uuid[16] = {0x1b, 0xc5, 0xd5, 0xa5, 0x02, 0x00, 0x36, 0xac, 0xe1, 0x11, 0x0E, 0x00, 0x01, 0x00, 0x00, 0x00};
static const uint8_t console_stderr_char_uuid[16] = {0x1b, 0xc5, 0xd5, 0xa5, 0x02, 0x00, 0x36, 0xac, 0xe1, 0x11, 0x0E, 0x00, 0x02, 0x00, 0x00, 0x00};
...
static const uint8_t config_mlc_char_uuid[16] = {0x1b, 0xc5, 0xd5, 0xa5, 0x02, 0x00, 0x36, 0xac, 0xe1, 0x11, 0x02, 0x00, 0x0F, 0x00, 0x00, 0x00};
...
tBleStatus Init_BlueNRG_Stack(void)
{
  ...
  ret = aci_gap_init(GAP_PERIPHERAL_ROLE, 0, strlen((char *)BoardName), &service_handle, &dev_name_char_handle, &appearance_char_handle);
...
#define CONFIG_DATA_RANDOM_ADDRESS          (0x80) /**< Stored static random address. Read-only. */
  ...
  ret = aci_gatt_update_char_value(service_handle, dev_name_char_handle, 0, strlen((char *) BoardName), (uint8_t *) BoardName);
...
tBleStatus Add_Config_Service(void)
{
  ...
  ret =  aci_gatt_add_char(configServiceHandle, UUID_TYPE_128, &char_mlc_uuid, 2 + 8 + 1/* 2 byte timestamp, 8 MLC registers output, 1 MCL output state */,
                           CHAR_PROP_NOTIFY,
...
static uint32_t BLE_ConfigConsole_SetRequests_Parsing(int8_t request, uint32_t len)
{
  switch (request)
  {
    ...
    case COM_REQUEST_MLC_CONFIG :
    {
      ...
      ISM330DHCX_GetUCF_FromBuffer(mlcConfigData, mlcConfigSize, ucfFileBuffer, ucfFileSize);
      ...
      if (osMessagePut(bleSendThreadQueue_id, COM_REQUEST_SENSORREFRESH | ISM330DHCX_Get_Id(), 0) != osOK)
      {
        SM_Error_Handler();
      }
      break;
...
/**
  * @brief  Write a chunk of the new FW to the flash and notify the app in case of error or if the procedure is complete
  * @param  buffer data
  * @param  len length of the data
...
static uint32_t BLE_OTA_upgrade_start(uint8_t *payload)
{
  ...
  else
  {
    ...
    {
      tBleStatus ret = aci_l2cap_connection_parameter_update_req(connection_handle, 10 /* interval_min*/, 10 /* interval_max */, 0 /* slave_latency */,
...
uint32_t SaveBoardNameToFlash(uint32_t address)
{
  ...
  {
    ValueToWrite = (((uint64_t) BoardName[4]) << (32 + 24)) | (((uint64_t) BoardName[5]) << (32 + 16)) | (((uint64_t) BoardName[6]) << (32 + 8))
                   | (((uint64_t) BoardName[0]) << (0 + 24)) | (((uint64_t) BoardName[1]) << (0 + 16)) | (((uint64_t) BoardName[2]) << (0 + 8))
