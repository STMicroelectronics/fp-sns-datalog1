...
static stmdev_ctx_t MLC_ctx_instance =
{
  SM_SPI1_Write_Os,
  SM_SPI1_Read_Os,
  NULL,
...
void Enable_Sensors(void)
{
  ...
  Activate_Sensor(LIS3DHH_Get_Id());
  Activate_Sensor(LIS2DW12_Get_Id());
  Activate_Sensor(LIS2MDL_Get_Id());
  Activate_Sensor(LSM6DSOX_Get_Id());
  Activate_Sensor(LPS22HH_Get_Id());
  Activate_Sensor(MP23ABS1_Get_Id());
...
void Activate_Sensor(uint32_t id)
{
  ...
  if (id == LSM6DSOX_Get_Id()) /* MLC subsensor should never start by default */
  {
    for (i = 0; i < 2; i++)
    {
...
void SDM_CalculateSdWriteBufferSize(COM_SubSensorStatus_t *pSubSensorStatus, uint32_t nBytesPerSample)
{
  ...
  /* fixedRAM = total miminum data used by the active subSensors. Each subSensor should use at least 
  SDM_MIN_BUFFER_SIZE bytes */
...
static uint32_t SDM_SaveData(void)
{
  ...
  for (ii = 0; ii < pDeviceDescriptor->nSensor; ii++)
  {
    pSensorDescriptor = COM_GetSensorDescriptor(ii);

    for (nn = 0; nn < pSensorDescriptor->nSubSensors; nn++)
    {
      if (COM_GetSubSensorStatus(ii, nn)->isActive)
      {
        SDM_Flush_Buffer(ii, nn);
        if (SDM_CloseFile(ii, nn) != 0)
        {
          return 1;
        }
      }
...
uint32_t readUCFfromSD(char *MLC_string)
{
  ...
  while (pch != NULL)
  {
    if (strncmp(pch, "Ac", 2) == 0)
    {
      ...
      lsm6dsox_write_reg(&MLC_ctx_instance, (uint8_t) ucf_reg, (uint8_t *) &ucf_data, 1);
    }
    pch = strtok(NULL, " -,_\r\n");
  }
...
void SDM_WriteUCF(char *ucfData, uint32_t ucfSize)
{
  ...
  if (BSP_SD_IsDetected())
  {
    ...
    f_open(&FileConfigMLC, "LSM6DSOX_MLC.ucf", FA_CREATE_ALWAYS | FA_WRITE);
