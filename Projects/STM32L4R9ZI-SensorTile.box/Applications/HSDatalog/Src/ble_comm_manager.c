...
extern lsm6dsox_mlc_status_mainpage_t mlc_status;
...
void BLE_CM_SPI_Init(void)
{
  ...
  hble_cm_spi.Init.CLKPolarity = SPI_POLARITY_LOW;
...
static uint32_t BLE_CM_DebugConsole_BuildResponse(uint32_t comRequest)
{
  switch (comRequest)
  {
    ...
    {
      aci_gatt_update_char_value(service_handle, dev_name_char_handle, 0, strlen((char *) BoardName),
                                 (uint8_t *) BoardName);
      break;
    }
...
static uint32_t BLE_CM_ConfigConsole_BuildResponse(uint32_t comRequest, char **pSerializedJson, int32_t *size)
{
  ...
  switch (comRequest)
  {
    case COM_REQUEST_DEVICE :
    {
      ...
      {
        serializedJsonSize = HSD_JSON_serialize_FWStatus_Logging(pSerializedJson, BSP_SD_IsDetected(),
                                                                 SD_Logging_Active);
      }
      break;
    }
    ...
    case COM_REQUEST_STATUS_PERFORMANCE :
    {
      ...
      serializedJsonSize = HSD_JSON_serialize_FWStatus_Performance(pSerializedJson, (char *) &BC_State.Name, mV,
                                                                   level, cpu_usage);
      break;
    }
